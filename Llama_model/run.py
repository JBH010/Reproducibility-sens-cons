...
# --- REPLACE THE OLD TimestampLogger WITH THIS NEW ONE ---

class TimestampLogger(object):
    """
    A custom logger that replaces sys.stdout and sys.stderr
    to prepend a timestamp to every line.
    
    This version includes fileno() and isatty() to be compatible
    with libraries like faulthandler (used by Ray).
    """
    def __init__(self, stream, log_type="STDOUT"):
        self.stream = stream
        self.log_type = log_type
        # Use a thread-local buffer to handle parallel prints
    ...
    def write(self, message):
        ...
        self.local.buffer += message
        lines = self.local.buffer.split('\n')
        self.local.buffer = lines[-1] # Keep last (incomplete) line

        for line in lines[:-1]:
            if line: # Don't print empty lines
                timestamp = datetime.now().strftime('%Y-%m-%d %H:%M:%S.%f')[:-3]
                log_prefix = f"[{timestamp}][{self.log_type}]"
                self.stream.write(f"{log_prefix} {line}\n")
    ...
    def flush(self):
        ...
        if hasattr(self.local, 'buffer') and self.local.buffer:
            timestamp = datetime.now().strftime('%Y-%m-%d %H:%M:%S.%f')[:-3]
            log_prefix = f"[{timestamp}][{self.log_type}]"
            self.stream.write(f"{log_prefix} {self.local.buffer}\n")
            self.local.buffer = ""

    # --- ✅ ADDED METHODS ---
    def fileno(self):
        """Pass fileno() call to the original stream."""
        return self.stream.fileno()

    def isatty(self):
        """Pass isatty() call to the original stream."""
        return self.stream.isatty()

# --- END OF REPLACEMENT BLOCK ---

# --- ✅ ADD THESE THREE LINES HERE ---
print("Redirecting stdout and stderr to TimestampLogger...")
sys.stdout = TimestampLogger(sys.stdout, "STDOUT")
sys.stderr = TimestampLogger(sys.stderr, "STDERR")
# --- END OF BLOCK TO ADD ---
...
def parse_DBPEDIA(input_file: Path, classes_file: Path) -> List[dict]:
    ...
    for idx, row in df.iterrows():
        new_entry = {
            ...
            "class": class_labels[row['Class']] if 'Class' in row else str(row.get('l1', ''))
        }
        data_list.append(new_entry)
...
def run_all_datasets(
    ...
    dataset_configs = {
        ...
        'TREC': {
            'data_file': Path('./data/TREC/test.txt'),
    ...
    for dataset_name in datasets:
        ...
        dataset_cfg['prompt_types'] = {
            ...
            if k in ['simple', 'instruct', 'fewshot']  # Note: it's 'fewshot' not '1-shot'
        }
